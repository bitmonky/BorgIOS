  ...
  tryRestoreState(){
    ...
    if (fs.existsSync(STATE_FILE)) {
      ...
      if (this.stateRestored){
        this.DMB  = this.trimBuffer(this.DMB,this.DMBMax);
 ...
 async buildPrompt(){
  ...
  doGetFileFromRepo(r, start, end) {
    return new Promise(async (resolve,reject)=>{
      ...
      try {
        ...
        this.respondEr("Error while retrieving file from repo... please try again.", r);
        resolve(false);
        return;
      }
    });
  ...
  doFetchCodeRepo(r) {
    return new Promise(async(resolve,reject)=>{
      ...
      console.log('REPOCODE result: ',this.DOCR);

      this.respondToBorg(r,"OK","to read a file use fetchRepoFile protocol.");
      resolve(true);
    });
  }
  ...
  respondToBorg(r,msg,tip=null){
    if (!tip){
      tip = "Repond in JSON only... All other text will be stripped out and disregarded by the API causing you to lose valuable contextual information! limit one req per response.";
    }
...
module.exports.BorgBrainModule = BorgBrainModule;
