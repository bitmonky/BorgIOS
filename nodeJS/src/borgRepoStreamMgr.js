...
function getDownloadTimeout(fileSizeBytes) {
    const sizeMB = fileSizeBytes / (1024 * 1024);
    
    if (sizeMB < 1) {
        // Tiny files: 5-10 seconds
        return 7000;
    } else if (sizeMB < 10) {
        // Small files: 15-30 seconds
        return Math.min(30000, sizeMB * 2000 + 10000);
    } else {
        // Large files: formula-based
        return Math.max(30000, sizeMB * 5000 + 30000);
    }
}
class BorgRepoStreamMgr {
  constructor(net) {
    this.net = net;
    this.cell = null;
  ...
  async doOpenStream(repo, service, winSize = 12) {
   ...
   //console.log(`doOpenStream():: repo.file`,j);
    ...
    return await this.streamComplete(fmap.streamId, getDownloadTimeout(fmap.totalSize));
  }
  streamComplete(streamId,tmax){
    let timer  = null;
    let listen = null;
    return new Promise((resolve) =>{
      this.net.on('streamDLoadFin',listen = (s) =>{
        if (s.streamId === streamId){
          clearTimeout(timer);
          this.net.removeListener('streamDLoadFin',listen);
          resolve(s);
          return;
        }
      });
      ...
      timer = setTimeout(() => {
        removeListerner(listen);
        resolve ({streamId: streamId,status: 'error',msg: 'download timeout'});
      },tmax);
    ...
    if (!result.ok) {
     //console.warn(`Shard ${idx} rejected for stream ${streamId}: ${result.reason}`);
      ...
      if (await this.maxTriesExceeded(stream,shardIdx)){
        this.net.emit(`streamDLoadFin`,{streamId: stream.streamId,status: 'error',msg: 'maxTries Exceeded'});
        return;
      }
...
module.exports.BorgRepoStreamMgr = BorgRepoStreamMgr;
