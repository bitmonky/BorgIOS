...
class BorgIOSmemoryMgr {
  constructor(net,borg) {
    this.net  = net;
    this.borg = borg;
    this.cell = null;
  ...
  async doStoreAgentMemory(agentMem){
     const fullMemory = this.net.deepClone(agentMem.memory);

     fullMemory.renderHtml = agentMem.renderHtml;
     const memStr  = JSON.stringify(fullMemory);
  ...
  async doOpenMemStream(memories, service, qry, winSize = 12) {
    ...
    const fmap = {
      ...
      nextToSend   : 0,
      service      : service,
      streamId     : this.net.calculateHash(JSON.stringify(memories)),
      filename     : this.net.calculateHash(qry),
...
module.exports.BorgIOSmemoryMgr = BorgIOSmemoryMgr;
